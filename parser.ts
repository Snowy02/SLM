...
// --- (Interfaces are unchanged, so they are omitted for brevity) ---
interface ParsedNode {
    id: string; // e.g., 'File:/path/to/file.ts' or 'Component:AppComponent:/path/to/component.ts'
    type: 'File' | 'Component' | 'Service' | 'Module' | 'Pipe' | 'Directive' | 'Interface' | 'Class' | 'Unknown';
    name: string;
...
// --- (Helper functions are unchanged, so they are omitted for brevity) ---
function getDecoratorName(decorator: ts.Decorator): string | undefined {
    const expression = decorator.expression;
    if (ts.isCallExpression(expression)) {
        return expression.expression.getText();
...
function parseAngularCode(projectPath: string, tsconfigPath?: string): Output {
    ...
    // --- FIX: Use a more reliable way to find the tsconfig file ---
    // The tsconfigPath should be relative to the projectPath, not the CWD.
    const resolvedTsConfigPath = tsconfigPath || path.join(projectPath, 'src', 'tsconfig.app.json');
    ...
    if (!configFileName) {
        // Fallback to the root tsconfig.json if the app one is not found.
        const fallbackConfig = path.join(projectPath, 'tsconfig.json');
        if (fs.existsSync(fallbackConfig)) {
            console.warn(`Could not find '${resolvedTsConfigPath}'. Falling back to '${fallbackConfig}'. This may not parse all application files.`);
            return parseAngularCode(projectPath, fallbackConfig);
        }
        throw new Error(`Could not find a valid 'tsconfig.json' or 'tsconfig.app.json' at '${resolvedTsConfigPath}'.`);
    }

    console.error(`Using tsconfig: ${configFileName}`); // Log which tsconfig is being used for debugging.
    ...
    // --- FIX: Normalize the project path for reliable comparison ---
    const normalizedProjectPath = path.normalize(path.resolve(projectPath)).toLowerCase();

    for (const sourceFile of allSourceFiles) {
        const normalizedSourcePath = path.normalize(sourceFile.fileName).toLowerCase();

        // --- FIX: Use the normalized paths for a reliable, case-insensitive check on all platforms ---
        if (sourceFile.isDeclarationFile || !normalizedSourcePath.startsWith(normalizedProjectPath)) {
            continue;
        }
        ...
        // --- (Rest of the function is the same, no changes needed below this point in this function) ---
        if (!fileMap.has(fileId)) {
            const fileNode: ParsedNode = {
                id: fileId,
                type: 'File',
        ...
        ts.forEachChild(sourceFile, (node) => {
            if (ts.isImportDeclaration(node)) {
                if (node.moduleSpecifier && ts.isStringLiteral(node.moduleSpecifier)) {
                    ...
                    if (!fileMap.has(targetFileId) && (resolvedImportPath.includes('.ts') || resolvedImportPath.includes('.js'))) {
                        const targetFileNode: ParsedNode = {
                            id: targetFileId,
                            type: 'File',
                            name: path.basename(resolvedImportPath),
                ...
                const className = node.name.text;
                const classId = `${className}:${relativeFilePath}`;
                ...
                if (decorators) {
                    for (const decorator of decorators) {
                        ...
                        if (decoratorName === 'Component' && decoratorArgs && decoratorArgs[0] && ts.isObjectLiteralExpression(decoratorArgs[0])) {
                            ...
                            const metadata = parseMetadata(decoratorArgs[0], sourceFile);
                            if (metadata.providedIn) entityProperties['providedIn'] = metadata.providedIn;
                        } else if (decoratorName === 'Injectable' && !decoratorArgs) {
                            entityType = 'Service';
                            entityId = `Service:${className}:${relativeFilePath}`;
                        }
                        else if (decoratorName === 'NgModule' && decoratorArgs && decoratorArgs[0] && ts.isObjectLiteralExpression(decoratorArgs[0])) {
                            ...
                            (metadata.imports as string[] || []).forEach(importName => {
                                entityRelationships.push({
                                    type: 'IMPORTS_MODULE',
                                    targetId: `Module:${importName}:UNKNOWN_PATH`
                                });
                            });
                            (metadata.providers as string[] || []).forEach(providerName => {
                                entityRelationships.push({
                                    type: 'PROVIDES',
                                    targetId: `Service:${providerName}:UNKNOWN_PATH`
                                });
                            });
                            (metadata.exports as string[] || []).forEach(exportName => {
                                entityRelationships.push({
                                    type: 'EXPORTS_MODULE',
                                    targetId: `UnknownExport:${exportName}:UNKNOWN_PATH`
                                });
                            });
                            (metadata.bootstrap as string[] || []).forEach(bootstrapComponentName => {
                                entityRelationships.push({
                                    type: 'BOOTSTRAPS',
                                    targetId: `Component:${bootstrapComponentName}:UNKNOWN_PATH`
                                });
                            });
                            if (metadata.id) entityProperties['moduleId'] = metadata.id;
                        ...
                        if (ts.isConstructorDeclaration(member) && member.parameters) {
                            member.parameters.forEach(param => {
                                if (param.type && param.name && ts.isIdentifier(param.name)) {
                                    ...
                                    entityRelationships.push({
                                        type: 'INJECTS',
                                        targetId: `Service:${paramTypeName}:UNKNOWN_PATH`,
                                        properties: { parameterName: paramName }
                                    });
                                }
                            });
                    ...
                    for (const clause of node.heritageClauses) {
                        if (clause.token === ts.SyntaxKind.ImplementsKeyword) {
                            for (const type of clause.types) {
                                ...
                                entityRelationships.push({
                                    type: 'IMPLEMENTS',
                                    targetId: `Interface:${implementedInterfaceName}:UNKNOWN_PATH`
                                });
                            }
                        }
                    }
    ...
    // Second pass to resolve relationship targetIds
    for (const node of nodes) {
        for (const rel of node.relationships) {
            const targetId = rel.targetId;
            if (targetId.endsWith(':UNKNOWN_PATH') || targetId.startsWith('UnknownExport:')) {
                const parts = targetId.split(':');
                ...
                const potentialTargets = nodes.filter(n =>
                    n.name === targetName &&
                    (targetTypeHint === 'UnknownExport' || n.id.startsWith(`${targetTypeHint}:`))
                );
...
if (require.main === module) {
    ...
    // --- FIX: Resolve paths correctly from command line arguments ---
    const projectPath = path.resolve(args[0]);
    // The tsconfig path argument should be resolved relative to the project path, not CWD.
    const tsConfigRelativePath = args[1]; // e.g., 'src/tsconfig.app.json'
    ...
    // The main function now handles finding the tsconfig file more robustly.
    // We pass the relative path if provided, otherwise it uses a smart default.
    const parsedData = parseAngularCode(projectPath, tsConfigRelativePath);
    console.log(JSON.stringify(parsedData, null, 2));
}