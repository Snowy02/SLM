Excellent observations. You're honing in on key usability details that significantly improve the application's professional feel and intuitive flow. I will address both of your points.

Guided Mode - "No Methods" Case: I will add logic to the class_action_selected_by_user function. If "SHOW METHODS" is selected for a class with no methods, it will now display an informative message and correctly hide the method selection and "Explain Code" sections, preventing user confusion.

NLP Mode - Progress & Result Order: I will fix the duplicate progress bar issue by defining the gr.Progress component once in the UI layout and passing it as an input to the handler. I will then convert the handle_nlp_query function into a generator using yield. This allows it to first show the "Query Execution Details" header and a loading message, then run the query, and finally yield a second update with the full results, ensuring the UI populates in the correct, logical order.

Overall Change Explanation

I've implemented two key UX improvements. First, in the "I am new..." path, the logic now checks if a class has methods; if not, it displays an informational message and hides the irrelevant UI elements instead of showing an empty list. Second, for the NLP path, I have refactored the interaction to be a multi-step process. A single, non-duplicating progress bar now appears, and the handle_nlp_query function uses yield to first update the UI with loading text and execution details, and only then, after the query completes, yields the final tabular results. This creates a more responsive, ordered, and professional user experience, eliminating duplicate progress bars and ensuring results appear in a logical sequence.
...
with gr.Blocks(theme=custom_theme) as demo:
    ...
    with gr.Row(visible=False) as nlp_mode_row:
        with gr.Column():
            ...
            # --- START: NLP UX ENHANCEMENT ---
            # 1. Explanation: The Progress component is defined once in the UI layout.
            # This prevents it from being re-rendered in multiple places on subsequent runs.
            nlp_progress = gr.Progress(track_tqdm=True, visible=False)
            # --- END: NLP UX ENHANCEMENT ---
    ...
    nlp_mode_components = [
        ...
        history_visible_state, nlp_dataframe_output, nlp_progress # Add new component to list
    ]
    ...
    # --- START: GUIDED MODE "NO METHODS" FIX ---
    # 2. Explanation: Added logic to handle the case where a class has no methods.
    def class_action_selected_by_user(repository, class_name, action):
        ...
        if action == "SHOW DEPENDENCIES":
            ...
            # 2a. Explanation: If the methods list is empty, show a message and hide the next steps.
            if not methods:
    ...
    # --- START: NLP UX ENHANCEMENT ---
    # 3. Explanation: This function is now a generator (using `yield`) to allow multi-stage UI updates.
    # It also no longer creates its own Progress instance.
    def handle_nlp_query(question, history, progress):
        ...
        # This initial yield makes the results area visible immediately with a loading state.
        yield {
        ...
        try:
            ...
            # Update UI after intent is identified
            yield {
                current_response_output: f"### Query Execution Details\n*Intent identified as '{intent}'. Generating query...*"
            }

            if intent == "dependency":
                result_data = query_handler.run_query(question, progress) # Pass progress for tqdm
        ...
        # Final yield with all the completed data
        yield {
            nlp_textbox: "",
            ...
            nlp_progress: gr.update(visible=False) # Hide the progress bar
        }
    # --- END: NLP UX ENHANCEMENT ---
    ...
    repository_radio.change(fn=repository_selected_by_user, inp
